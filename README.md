...
- **Recovery plane — USB-C.** Composite USB gadget (ECM + ACM serial) under its own systemd unit, independent of robot firmware. The workbench exposes an xterm.js terminal over this.
...
ESP32 and Pi expose the same service UUID and characteristic UUIDs, so the workbench talks to either without conditional logic. `docs/` is the GitHub Pages publish root — the site is the directory, no build step.
