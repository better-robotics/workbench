...
// Complementary filter weight on the gyro side. At 100 Hz:
//   0.98 means accel corrects ~2% of angle each tick → full correction in ~50 ticks.
#define ALPHA 0.98f
...
esp_err_t imu_init(i2c_port_t port, int sda_pin, int scl_pin, uint8_t addr) {
    ...
    // Seed the complementary filter with a static accel reading so the
    // first few loop ticks don't snap from 0° to the real angle.
    uint8_t raw[6];
    if (mpu_read_regs(REG_ACCEL_XOUT_H, raw, 6) == ESP_OK) {
        ...
        s_pitch = atan2f(ax_g, az_g) * (180.0f / (float)M_PI);
    ...
    s_ready = true;
    ESP_LOGI(TAG, "MPU6050 ready at 0x%02x — initial pitch %.2f°", addr, s_pitch);
    return ESP_OK;
}
...
void imu_update(float dt_s) {
    ...
    float gy_dps = gy_raw / GYRO_SCALE;    // degrees/second

    // Accel-derived angle — stable long-term, noisy short-term.
    float accel_pitch = atan2f(ax_g, az_g) * (180.0f / (float)M_PI);

    // Complementary filter: gyro for fast dynamics, accel for drift correction.
    s_pitch = ALPHA * (s_pitch + gy_dps * dt_s) + (1.0f - ALPHA) * accel_pitch;
